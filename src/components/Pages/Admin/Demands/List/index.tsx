...
const DemandListPage = memo(() => {
  ...
  const formCallback = useCallback(
    (demand?: IDemand) => {
      setFormOpened(false);
      current ? refresh() : mergeParams({ term: demand.name });
    },
  ...
  return (
    <Fragment>
      ...
      <Card>
        <FormDialog opened={formOpened} demand={current} onComplete={formCallback} onCancel={formCancel} />
        ...
        <TableWrapper minWidth={500}>
          <Table>
            ...
            <TableBody>
              ...
              />
              {results.map(demand => (
                <ListItem key={demand.id} demand={demand} onEdit={handleEdit} onDeleteComplete={refresh} />
              ))}
            </TableBody>
          </Table>
        </TableWrapper>
